...
import { ActivityType, ColumnDataType, ChecklistType, ColumnItemType, CardType, ColumnsType, MainProps } from './types';

const Board = (props: MainProps) => {
  const { enqueueSnackbar } = useSnackbar();

  const [state, setState] = useState<ColumnsType>({
    columns: {},
    columnOrder: [],
    archive: {
      _id: 'archive',
      ...
      .then(
        (data: {
          ...
          if (data.success) {
            ...
            data.columns.map((column: ColumnItemType) => {
  ...
  const addColumn = (sectionTitle: string, card: CardType | undefined) => {
  ...
  const updateCard = (
    ...
    fetch(`/api/update-card`, {
      ...
      .then((res) => res.json())
      .then((data: { success: boolean; data: CardType }) => {
        if (data.success) {
          state.columns[columnId].cards.map((item: CardType, index: number) => {
            if (item._id === card._id) {
              ...
              if (card.isArchived) {
                let tempCard: CardType = {} as CardType;
  ...
  const deleteCard = (columnId: string, cardId: string) => {
    fetch(`/api/delete-card`, {
      ...
      .then((data: { success: boolean; message: string }) => {
        if (data.success) {
          state.columns[columnId].cards.map((card: CardType, index: number) => {
            if (card._id === cardId) {
              state.columns[columnId].cards.splice(index, 1);
            }
          });
  ...
  const reorderCards = (column: ColumnItemType, cards: []) => {
  ...
  const addChecklist = (
    ...
    fetch(`/api/add-checklist`, {
      ...
      .then((res) => res.json())
      .then((data: { success: boolean; data: ChecklistType }) => {
        if (data.success) {
          state.columns[columnId].cards.map((card: CardType) => {
            if (card._id === cardId) {
          ...
          state.columns[columnId].cards.map((card: CardType) => {
            if (card._id === cardId) {
              if (checklist.checklist) {
                card.checklists.map((item: ChecklistType) => {
          ...
          state.columns[columnId].cards.map((card: CardType) => {
            if (card._id === cardId) {
              card.checklists = card.checklists.filter(
                (checklist: ChecklistType) => checklist._id !== checklistId
              );
            }
          });
          updateDate(columnId, cardId);
  ...
  const addActivity = (columnId: string, cardId: string, activity: string) => {
    fetch(`/api/add-activity`, {
      ...
      .then((res) => res.json())
      .then((data: { success: boolean; data: ActivityType }) => {
        if (data.success) {
          if (columnId === archiveColumnId) {
            state.archive.cards.map((card: CardType) => {
              if (card._id === cardId) {
